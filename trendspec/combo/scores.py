"""组合打分唯一实现（trendspec.combo.scores）。

`compute_combo_scores` 是 group_by 分组解析 + 去极值(winsorize) + z-score
加权的唯一实现——FactorStrategy.init() 与 research 快评估（fast_eval）
均应调用本模块，不要在别处重新实现。历史上曾有无 winsorize/group_by 的
简化重实现，与 FactorStrategy 内联逻辑不一致，导致 precomputed_scores
注入与批量评估结果偏离真实回测（2026-07-11 修复）。
